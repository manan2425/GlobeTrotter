...
export function seedData() {
  ...
  const stmtAch = db.prepare(`INSERT OR REPLACE INTO achievements (id, code, title, description, icon, badge_color) VALUES (?, ?, ?, ?, ?, ?)`);
  for (const ach of achievements) {
    stmtAch.run(ach.id, ach.code, ach.title, ach.description, ach.icon, ach.badge_color);
  }
  ...
  const stmtUser = db.prepare(`INSERT OR REPLACE INTO users (id, email, password_hash, full_name, profile_photo, role, currency, language) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
  stmtUser.run('u_manan', 'manan@globetrotter.com', passwordHash, 'Manan Patel', 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80', 'user', 'INR', 'en');
  stmtUser.run('u_admin', 'admin@globetrotter.com', adminPasswordHash, 'Admin GlobeTrotter', 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=400&q=80', 'admin', 'INR', 'en');
  stmtUser.run('u_priya', 'priya@globetrotter.com', passwordHash, 'Priya Sharma', 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=400&q=80', 'user', 'INR', 'en');

  // Profiles
  const stmtProfile = db.prepare(`INSERT OR REPLACE INTO profiles (id, user_id, bio, home_city, home_country, is_public, public_trips) VALUES (?, ?, ?, ?, ?, ?, ?)`);
  stmtProfile.run('prof_manan', 'u_manan', 'Passionate explorer, backpacker & food enthusiast! 🌍✈️', 'Ahmedabad', 'India', 1, 1);
  stmtProfile.run('prof_admin', 'u_admin', 'GlobeTrotter Platform Admin & Community Lead', 'Mumbai', 'India', 1, 1);

  // User Achievements
  const stmtUserAch = db.prepare(`INSERT OR REPLACE INTO user_achievements (id, user_id, achievement_id) VALUES (?, ?, ?)`);
  stmtUserAch.run('ua_1', 'u_manan', 'ach_1');
  stmtUserAch.run('ua_2', 'u_manan', 'ach_3');
  stmtUserAch.run('ua_3', 'u_manan', 'ach_4');
  ...
  const stmtCountry = db.prepare(`INSERT OR REPLACE INTO countries (id, name, code, currency, region) VALUES (?, ?, ?, ?, ?)`);
  for (const c of countries) {
    stmtCountry.run(c.id, c.name, c.code, c.currency, c.region);
  }
  ...
  const stmtCity = db.prepare(`INSERT OR REPLACE INTO cities (id, name, country_id, country_name, region, description, image_url, popularity_score, avg_daily_cost, best_time_to_visit, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const c of cities) {
    stmtCity.run(c.id, c.name, c.country_id, c.country_name, c.region, c.description, c.image_url, c.popularity_score, c.avg_daily_cost, c.best_time_to_visit, c.latitude, c.longitude);
  }
  ...
  const stmtAct = db.prepare(`INSERT OR REPLACE INTO activities (id, city_id, name, category, description, image_url, location_name, latitude, longitude, duration_minutes, estimated_cost, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const a of activities) {
    stmtAct.run(a.id, a.city_id, a.name, a.category, a.description, a.image_url, a.location_name, a.latitude, a.longitude, a.duration_minutes, a.estimated_cost, a.rating);
  }
  ...
  const stmtTpl = db.prepare(`INSERT OR REPLACE INTO trip_templates (id, title, description, category, cover_image, duration_days, estimated_budget, currency, template_data_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const t of templates) {
    stmtTpl.run(t.id, t.title, t.description, t.category, t.cover_image, t.duration_days, t.estimated_budget, t.currency, t.template_data_json);
  }

  // 6. Create Sample Trip: "Rajasthan Adventure" for Manan
  const tripId = 'trip_rajasthan_demo';
  const stmtTrip = db.prepare(`INSERT OR REPLACE INTO trips (id, user_id, title, description, cover_image, start_date, end_date, estimated_budget, currency, status, visibility, public_slug) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  stmtTrip.run(
    tripId,
    'u_manan',
    'Rajasthan Adventure',
  ...
  // Trip Stops
  const stmtStop = db.prepare(`INSERT OR REPLACE INTO trip_stops (id, trip_id, city_id, stop_order, arrival_date, departure_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`);
  stmtStop.run('stop_1', tripId, 'city_amd', 1, '2026-09-10', '2026-09-11', 'Starting point - historic walks');
  stmtStop.run('stop_2', tripId, 'city_udaipur', 2, '2026-09-11', '2026-09-13', 'City of Lakes - palace stay');
  stmtStop.run('stop_3', tripId, 'city_jodhpur', 3, '2026-09-13', '2026-09-14', 'Blue city fort ziplining');
  stmtStop.run('stop_4', tripId, 'city_jaipur', 4, '2026-09-14', '2026-09-16', 'Pink city heritage & bazaars');

  // Trip Activities
  const stmtTripAct = db.prepare(`INSERT OR REPLACE INTO trip_activities (id, trip_id, trip_stop_id, activity_id, day_number, custom_title, category, time_slot, duration_minutes, cost, notes, activity_order, is_completed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  stmtTripAct.run('tact_1', tripId, 'stop_1', 'act_amd_1', 1, 'Sabarmati Ashram Walk', 'Culture', '09:00', 90, 0, 'Morning peaceful walk', 1, 1);
  stmtTripAct.run('tact_2', tripId, 'stop_1', 'act_amd_2', 1, 'Manek Chowk Night Food Tour', 'Food', '20:00', 120, 400, 'Try the butter cheese dosa', 2, 0);

  stmtTripAct.run('tact_3', tripId, 'stop_2', 'act_udr_1', 2, 'City Palace Tour', 'Sightseeing', '10:00', 150, 400, 'Audio guide recommended', 1, 0);
  stmtTripAct.run('tact_4', tripId, 'stop_2', 'act_udr_2', 2, 'Lake Pichola Sunset Boat Cruise', 'Adventure', '17:30', 60, 800, 'Reach ghat 20 mins early', 2, 0);
  stmtTripAct.run('tact_5', tripId, 'stop_2', 'act_udr_3', 3, 'Sajjangarh Monsoon Palace', 'Nature', '16:00', 120, 250, 'Catch sunset view', 1, 0);

  stmtTripAct.run('tact_6', tripId, 'stop_3', 'act_jdh_1', 4, 'Mehrangarh Fort Zipline', 'Adventure', '09:30', 180, 1800, 'Book slots in advance', 1, 0);
  stmtTripAct.run('tact_7', tripId, 'stop_3', 'act_jdh_2', 4, 'Blue City Walk & Lassi', 'Culture', '15:00', 120, 300, 'Shri Mishrilal Hotel for Lassi', 2, 0);

  stmtTripAct.run('tact_8', tripId, 'stop_4', 'act_jpr_1', 5, 'Amber Fort Safari & Sheesh Mahal', 'Sightseeing', '09:00', 180, 500, 'Guide hired at entrance', 1, 0);
  stmtTripAct.run('tact_9', tripId, 'stop_4', 'act_jpr_2', 5, 'Hawa Mahal Photo Spot', 'Culture', '15:30', 60, 200, 'Tattoo Cafe rooftop view', 2, 0);
  stmtTripAct.run('tact_10', tripId, 'stop_4', 'act_jpr_3', 6, 'Chokhi Dhani Village Experience', 'Food', '18:30', 180, 1100, 'Traditional Rajasthani Thali dinner', 1, 0);

  // Accommodations
  const stmtAcc = db.prepare(`INSERT OR REPLACE INTO accommodations (id, trip_id, trip_stop_id, name, check_in, check_out, cost_per_night, total_cost, booking_reference, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  stmtAcc.run('acc_1', tripId, 'stop_1', 'Lemon Tree Hotel Ahmedabad', '2026-09-10', '2026-09-11', 3200, 3200, 'BK-88492', 'Mithakhali Six Roads');
  stmtAcc.run('acc_2', tripId, 'stop_2', 'Jagat Niwas Palace Hotel Udaipur', '2026-09-11', '2026-09-13', 4500, 9000, 'BK-99301', 'Lal Ghat, Lake Pichola');
  stmtAcc.run('acc_3', tripId, 'stop_3', 'RAAS Jodhpur Boutique Hotel', '2026-09-13', '2026-09-14', 5000, 5000, 'BK-10294', 'Tunwar ji ka Jhalra');
  stmtAcc.run('acc_4', tripId, 'stop_4', 'Shahpura House Jaipur', '2026-09-14', '2026-09-16', 3800, 7600, 'BK-44821', 'Devi Marg, Bani Park');

  // Expenses
  const stmtExp = db.prepare(`INSERT OR REPLACE INTO expenses (id, trip_id, user_id, category, amount, date, description, paid_by_name, payment_method) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  stmtExp.run('exp_1', tripId, 'u_manan', 'Transportation', 2500, '2026-09-10', 'Vande Bharat Express Ahmedabad to Udaipur', 'Manan Patel', 'UPI');
  stmtExp.run('exp_2', tripId, 'u_manan', 'Accommodation', 3200, '2026-09-10', 'Hotel stay Ahmedabad', 'Manan Patel', 'Card');
  stmtExp.run('exp_3', tripId, 'u_manan', 'Activities', 400, '2026-09-10', 'Manek Chowk Street Food Dinner', 'Priya Sharma', 'Cash');
  stmtExp.run('exp_4', tripId, 'u_manan', 'Activities', 1200, '2026-09-11', 'City Palace Tickets & Boat Cruise', 'Manan Patel', 'UPI');

  // Trip Members
  const stmtMem = db.prepare(`INSERT OR REPLACE INTO trip_members (id, trip_id, user_id, email, role, status) VALUES (?, ?, ?, ?, ?, ?)`);
  stmtMem.run('mem_1', tripId, 'u_manan', 'manan@globetrotter.com', 'Owner', 'Accepted');
  stmtMem.run('mem_2', tripId, 'u_priya', 'priya@globetrotter.com', 'Editor', 'Accepted');

  // Comments
  const stmtComm = db.prepare(`INSERT OR REPLACE INTO comments (id, trip_id, user_id, activity_id, content) VALUES (?, ?, ?, ?, ?)`);
  stmtComm.run('comm_1', tripId, 'u_priya', 'act_udr_2', 'Should we book the private sunset boat or group cruise? Sunset views are magical!');
  stmtComm.run('comm_2', tripId, 'u_manan', 'act_jdh_1', 'Ziplining over Mehrangarh fort looks epic! Reserved the 09:30 AM slot.');

  // Activity Votes
  const stmtVote = db.prepare(`INSERT OR REPLACE INTO activity_votes (id, trip_activity_id, user_id, vote_type) VALUES (?, ?, ?, ?)`);
  stmtVote.run('vote_1', 'tact_4', 'u_manan', 'up');
  stmtVote.run('vote_2', 'tact_4', 'u_priya', 'up');
  stmtVote.run('vote_3', 'tact_6', 'u_manan', 'up');
  stmtVote.run('vote_4', 'tact_6', 'u_priya', 'up');

  // Notifications
  const stmtNotif = db.prepare(`INSERT OR REPLACE INTO notifications (id, user_id, title, message, type, is_read, link_url) VALUES (?, ?, ?, ?, ?, ?, ?)`);
  stmtNotif.run('notif_1', 'u_manan', 'Trip Reminder ✈️', 'Your Rajasthan Adventure starts in 18 days! Complete your packing list.', 'reminder', 0, '/trips/trip_rajasthan_demo');
  stmtNotif.run('notif_2', 'u_manan', 'Weather Update ☀️', 'Sunny weather expected in Udaipur (28°C - 32°C). High UV index.', 'weather', 0, '/trips/trip_rajasthan_demo');
  stmtNotif.run('notif_3', 'u_manan', 'Priya joined your trip', 'Priya Sharma accepted your invite to edit Rajasthan Adventure.', 'invite', 1, '/trips/trip_rajasthan_demo');

  console.log('✅ Database seeded with realistic cities, activities, templates, user, and trips!');
}
