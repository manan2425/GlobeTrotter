...
router.post('/templates/:id/use', authenticateToken, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    if (tplData.cities && Array.isArray(tplData.cities)) {
      ...
      for (let idx = 0; idx < tplData.cities.length; idx++) {
        const cityId = tplData.cities[idx];
        await stmtStop.run(`stop_${Date.now()}_${idx}`, tripId, cityId, idx + 1, startDate, endDate, `Template stop ${idx + 1}`);
      }
    }
