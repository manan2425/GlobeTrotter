...
router.post('/trips/:id/share', authenticateToken, (req: AuthRequest, res: Response) => {
...
router.get('/public/trips/:slug', (req, res) => {
  ...
  const accommodations = db.prepare('SELECT * FROM accommodations WHERE trip_id = ?').all(trip.id);
  const transportation = db.prepare('SELECT * FROM transportation WHERE trip_id = ?').all(trip.id);
...
router.post('/public/trips/:slug/copy', authenticateToken, (req: AuthRequest, res: Response) => {
  const slug = req.params.slug;
  const userId = req.user!.id;

  const sourceTrip: any = db.prepare(`SELECT * FROM trips WHERE public_slug = ? AND visibility = 'Public'`).get(slug);
  if (!sourceTrip) {
  ...
  // Duplicate stops
  const stops = db.prepare('SELECT * FROM trip_stops WHERE trip_id = ?').all(sourceTrip.id);
  ...
  const activities = db.prepare('SELECT * FROM trip_activities WHERE trip_id = ?').all(sourceTrip.id);
